...
function processV1Request (request, response) {
  ...
  console.log('User Info: ',app.getUser())
  const actionHandlers = {
    ...
    'turnLightOnInRoom': () => {
      ...
      axios({
        ...
        method: 'get',
        headers: {
            // TODO: Use the user access token instead of a general access token.
        ...
        if (
          ...
          axios({
            url: functions.config().billiam.url + '/?uid=' + socketOfInterest.uid,
            method: 'get',
            headers: {
                authorization: `Bearer ${functions.config().billiam.key}`
            }
          }).then(res => {
            console.log('switch signal sent')
